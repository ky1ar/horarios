...
<body>
    ...
    <header>
        ...
        <section id="timeline">
            ...
            $sql = "SELECT s.id_schedule, c.id_date, c.calendar_date, s.stamp, s.id_user FROM Calendar c LEFT JOIN Schedule s ON s.id_calendar = c.id_date AND s.id_user = 13 WHERE c.id_date BETWEEN $start_date_id AND ($start_date_id + $total_days);";
            $result = $conn->query($sql);
            while ($row = $result->fetch_assoc()){
