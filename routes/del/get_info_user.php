...
if (isset($_POST['userId']) && isset($_POST['month']) && isset($_POST['year'])) {
    ...
    // Preparar la consulta SELECT SQL
    $sql = "SELECT
                SUM(
                    CASE
    ...
    // Ejecutar la consulta con multi_query
    if ($conn->multi_query($sql)) {
        // Inicializar el array de datos finales
        $finalData = array();

        // Obtener los resultados y agregarlos al array finalData
        do {
        ...
        // Si hay un error en la consulta, imprimir un mensaje de error
        echo json_encode(array('success' => false, 'message' => 'Error al ejecutar la consulta.'));
    }
} else {
    // Si no se recibieron los parámetros necesarios, imprimir un mensaje de error
