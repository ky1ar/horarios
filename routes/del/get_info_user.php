...
if (isset($_POST['userId']) && isset($_POST['month']) && isset($_POST['year'])) {
    ...
    // $conn->query("SET 2024 = $year");
    // $conn->query("SET 05 = $month");
...
FROM
    Calendar c
JOIN
    Users u ON u.id_user = 18
WHERE
    c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 05, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 05, '-01')))
    AND c.holiday = 0;
...
JOIN
    Users u ON u.id_user = 18
LEFT JOIN
    Schedule s ON c.id_date = s.id_calendar AND s.id_user = 18
WHERE
    c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 05, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 05, '-01')))
