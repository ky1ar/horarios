    ...
    WHEN SUM(
            ROUND(
                CASE
                    ...
                    WHEN u.id_profile = 2 AND DAYOFWEEK(c.calendar_date) BETWEEN 2 AND 6 AND c.calendar_date < (SELECT DATE_SUB(MAX(stamp_date), INTERVAL 1 DAY) FROM Archivos) THEN GREATEST(0, (20 - COALESCE(LENGTH(s.stamp), 0)) / 5)
                    WHEN u.id_profile = 2 AND DAYOFWEEK(c.calendar_date) = 7 AND c.calendar_date < (SELECT DATE_SUB(MAX(stamp_date), INTERVAL 1 DAY) FROM Archivos) THEN GREATEST(0, (10 - COALESCE(LENGTH(s.stamp), 0)) / 5)
