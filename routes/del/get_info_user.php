...
if (isset($_POST['userId']) && isset($_POST['month']) && isset($_POST['year'])) {
    ...
    FROM
        Calendar c
    JOIN
        Users u ON u.id_user = 18 -- @id_user
    WHERE
        c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 5, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 5, '-01')))
        AND c.holiday = 0) AS total_hours_required,
    ...
    JOIN
        Users u ON u.id_user = 18 -- @id_user
    LEFT JOIN
        Schedule s ON c.id_date = s.id_calendar AND s.id_user = 18 -- @id_user
    WHERE
        c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 5, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 5, '-01')))
        AND c.calendar_date < CURDATE() -- @current_date
        AND c.holiday = 0) AS total_missing_points,
    ...
    WHERE
        c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 5, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 5, '-01')))
        AND c.holiday = 0) AS total_late_points,
    ...
    WHERE
        c.calendar_date BETWEEN DATE(CONCAT(2024, '-', 5, '-01')) AND LAST_DAY(DATE(CONCAT(2024, '-', 5, '-01')))
        AND c.holiday = 0) AS total_unjustified_absences
FROM
    Users u
WHERE
    u.id_user = 18";
