      ...
      success: function (response) {
        if (response.success) {
          ...
          response.schedule.forEach(function (entry, index) {
            var dayIndex = daysOfWeek.indexOf(entry.day_name_espanol);
            if (dayIndex === -1) {
              console.error("Día inválido:", entry.day_name_espanol);
              return;
            }
            ...
            if (index % 6 === 0) {
              ...
              // Reset weekDays array for each new week
              weekDays = [];
            }
  
            // Fill missing days if any
            while (daysOfWeek.indexOf(weekDays[daysCounter % 7]) !== dayIndex) {
              var missingDayIndex = daysOfWeek.indexOf(weekDays[daysCounter % 7]);
              var missingDayName = daysOfWeek[missingDayIndex];
              $("<li class='day-nam'>" + missingDayName + " " + "</li>").appendTo($currentHrrBox.find(".hrr-day"));
              $("<ul></ul>").appendTo($currentHrrBox.find(".hrr-day"));
              daysCounter++;
            }
            ...
            weekDays.push(entry.day_name_espanol);
            daysCounter++;
          });
  
          // Fill missing days for the remaining week
          while (daysCounter % 6 !== 0) {
            var missingDayIndex = daysOfWeek.indexOf(weekDays[daysCounter % 7]);
            var missingDayName = daysOfWeek[missingDayIndex];
            $("<li class='day-nam'>" + missingDayName + " " + "</li>").appendTo($currentHrrBox.find(".hrr-day"));
            $("<ul></ul>").appendTo($currentHrrBox.find(".hrr-day"));
