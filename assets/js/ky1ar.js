  ...
  function calcularSumaCalcPorSemana(userId, year, month) {
    ...
    $(".hrr-box").each(function (index) {
      ...
      getWeeklyData(userId, semana, year, month, function (acumuladoValorDia, idProfile) {
        ...
        // Imprimir el resultado en la consola
        console.log(
          "Id_Profile " + idProfile + ", Semana " + semana + ", suma calc: " + resultado + ", Valor acumulado " + acumuladoValorDia + ", DF count: " + dfCount + ", DF dates: " +
          dfDates.map((date) => {
        ...
        function sumarRestarHoras(totalMinutosActual, resultado, restar) {
          ...
          const nuevaHora = `${Math.floor(nuevoTotalMinutos / 60)}:${Math.abs(nuevoTotalMinutos % 60).toString().padStart(2, "0")}`;
        ...
        if (resultado.includes("-")) {
          ...
          const nuevaHoraSuma = sumarRestarHoras(acumuladoValorDia.toString(), resultado, false);
