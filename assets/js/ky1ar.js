$(document).ready(function () {
  ...
  function obtenerDiaDeLaSemana(fechaString) {
    ...
    const diaDeLaSemana = fecha.getDay() - 1; // Ajustar para que 0: Lunes, 1: Martes, ..., 5: Sábado
    ...
    $(".hrr-box").each(function (index) {
        ...
        getWeeklyData(userId, semana, year, month, function (acumuladoValorDia) {
            $hrrBox.find(".calc").each(function () {
                ...
                if (mesCalc === month && diaDeLaSemana >= 1 && diaDeLaSemana <= 6 && fecha.getFullYear() === year) { // Solo lunes a sábado y dentro del mismo mes y año
                    if (calc === "DF") {
                        dfDetails.push({
